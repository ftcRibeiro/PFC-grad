...
\textit{Container} é uma unidade de software para desenvolvimento e implantação que empacota código e suas dependências para a execução de um aplicativo de forma rápida, escalável e confiável, independentemente de do ambiente de computação que o hospede. No projeto descrito por essa monografia foram utilizados \textit{containers docker} \cite{what-container}.
...
\textit{KPIs}, sigla do inglês para \textit{Key Performance Indicators}, são indicadores projetados para medir objetivos críticos de um sistema geralmente complexo, indo de uma simples malha de controle regulatório, a uma inteira linha de montagem automotiva. O monitoramento desse tipo de indicadores promove o aumento no entendimento do sistema a ser analisado, auxiliando em possíveis tomadas de decisões estratégicas, e aumento de produtividade \cite{kpi1_ref1}. O controle desses índices busca geralmente melhorar qualidade e produtividade, alcançando pontos ótimos de operação, bem como aumento da lucratividade da cadeia produtiva em questão. Um exemplo de aplicação de controle de KPIs pode ser visto em \cite{kpi_tolerance}. 


\subsection{Controller Performance Monitoring - CPM}

\textit{CPM}, sigla do inglês para \textit{Controller Performance Monitoring} é a técnica mais usada para a medição de \textit{KPIs} direcionados a sistemas de automação.

São softwares que monitoram, identificam, e geram diagnósticos e sugestões para problemas em controle de ativos industriais, \cite{automation_kpis}
\todo[inline]{Detalhar sobre a utilização de KPIs para análise de processos industriais. Falar aqui também sobre PIMS;}
\section{Trabalhos Relacionados}
\todo[inline]{Falar sobre o trabalho que está no zotero sobre criação de aplicação, depois falar sobre o artigo de kpis e partir pra ideia de ter um pouco de cada como referências;}
