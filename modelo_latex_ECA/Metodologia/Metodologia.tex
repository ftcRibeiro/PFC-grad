...
A primeira etapa de validações consistiu na execução manual de um único ciclo do sistema. Após o término, foram coletados os resultados dos cálculos para as malhas escolhidas. Para apoio e referência, foi utilizada uma planilha de \textit{excel} para realização dos cálculos das mesmas malhas. A tabela \ref{} traz os valores obtidos em cada uma das operações. 
...
\begin{table}
	\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
		...
		-       & \textit{Excel}               & \textit{Kpi-Executor} & \textit{Excel}                & \textit{Kpi-Executor} & \textit{Excel}                & \textit{Kpi-Executor} & \textit{Excel}       & \textit{Kpi-Executor} \\ \hline
		Malha 1 & ~                   & ~            & ~                    & ~            & ~                    & ~            & ~           & ~            \\ \hline
		Malha 2 & ~                   & ~            & ~                    & ~            & ~                    & ~            & ~           & ~            \\ \hline
	\end{tabular}
\end{table}
...
\todo[inline]{adicionar a descrição do processo de testes no ambiente de QA: validação de kpi a kpi exibido no loop. Aqui podemos adicionar prints do loop}
...
\begin{itemize}
	...
	Os testes deste item foram realizados pelo \textit{Product Owner} do Loop, validando o funcionamento dos pontos descritos acima.
	...
	\subitem \textit{Tela de Diagnóstico:} Nessa tela são exibidos os dados de processo e índices de performance da malha selecionada em um determinado período de tempo. Foi escolhida, de forma aleatória, uma malha para o período de tempo de $03/02/2020 15:00$ a $03/02/2020 16:00$ 
	
	\begin{figure}[!htbp]
		\centering		
		\includegraphics[width=14cm]{Metodologia/Figuras/telaDiagnostico.png}
...
\todo[inline]{Descrever as etapas de dispoibilização no ambiente do cliente. Substituição in live do módulo anterior}
