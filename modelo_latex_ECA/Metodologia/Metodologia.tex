...
\paragraph{Validação dos Cálculos}

A primeira etapa de validações consistiu na execução manual de um único ciclo do sistema. Após o término, foram coletados os resultados dos cálculos para as malhas escolhidas. Para apoio e referência, foi utilizada uma planilha de \textit{excel} para realização dos cálculos das mesmas malhas, no período de tempo de 06/03/2020 15:00 a 06/03/2020 16:00. 
...
\paragraph{Validação em Ambiente de Testes}
...
\begin{itemize}
	\item \textbf{Implantação no ambiente de teste:} Construção de \textit{Dockerfile} e configuração de orquestrador de containers (Kubernetes);
	\item \textbf{Validação ciclicidade:} Foi avaliado o funcionamento cíclico do sistema, sendo observadas, através de consultas ao banco de dados no qual são registrados os cálculos dos \textit{KPIs}, novas execuções do \textit{Kpi-Executor} a cada hora;
	...
	Os testes deste item foram realizados pelo \textit{Product Owner} do Loop, utilizando o período de 06/03/2020 15:00 a 06/03/2020 16:00, validando o funcionamento dos pontos a seguir.
	...
	\subsubitem 1. O número de malhas \textit{offline} deveria se manter no valor de 2, para o período de tempo exibido na figura \ref{fig:telaDashboard};
	\subsubitem 2. O número de malhas em automático deveria mostrar coerente ao registrado no banco para aquele período em questão;
	\subsubitem 3. O número de malhas em modo manual deveria se mostrar coerente ao registrado em banco para aquele período em questão;
	\subsubitem 4. Os valores de \textit{KPIs} exibidos na tabela indicada deveriam estar coerentes com os registros inseridos pelo \textit{Kpi-Executor} no banco de cálculos. Para essa validação, foram escolhidas malhas arbitrariamente, seus\textit{KPIs} foram calculados de forma manual para o período de tempo em questão, e esses valores foram comparados aos exibidos nesse ponto da tela.
	...
	\subsubitem 1. Nomes das malhas selecionadas para comparação;
	\subsubitem 2. Legenda dos \textit{KPIs} selecionados para comparação;
	\subsubitem 3. Lista dos \textit{KPIs} selecionados.
...
\paragraph{Disponibilização em Ambiente de Produção}
\todo[inline]{Descrever as etapas de disponibilização no ambiente do cliente. Substituição in live do módulo anterior}
